...
experiment = 'HSV Rabies/HSV_HEK_Transfection_14_Jun2018'
virus = 'N2c'
file_type = '_I_'
month = '2018'
...
for b in range(len(threshold)):
	for f in range(len(extension)):
		for p in range(len(days)):
		    ...
		    result_fol_step_3 = result_fol_step_2  + month + days[p] + file_type + virus + '_' + experiment_type +'/' + threshold[b] + "/"
		    ...
		    for q in range(len(image_name)):
		    	...
		    	IJ.selectWindow( 'ST_' + month + days[p] + file_type + virus + '_' + experiment_type + "\\" + image_name[q]);
		    	IJ.run("Close");
		    	IJ.selectWindow("Drawing of " +  'ST_' + month + days[p] + file_type + virus + '_' + experiment_type + "\\" + image_name[q]);
		    	IJ.run("Invert");
		    	...
		    	IJ.selectWindow( 'ST_' + month + days[p] + file_type + virus + '_' + experiment_type + "\\" + image_name[q])
		    	IJ.run("Add Image...", "image=[Drawing of " +  'ST_' + month + days[p] + file_type + virus + '_' + experiment_type + "\\" + image_name[q] + "] x=0 y=0 opacity=90 zero");
		    	IJ.selectWindow( 'ST_' + month + days[p] + file_type + virus + '_' + experiment_type + "\\" + image_name[q])	
		    	imp3 = imp2.flatten();
